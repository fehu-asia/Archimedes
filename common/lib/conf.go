...
type RedisConf struct {
	...
	Network     string   `mapstructure:"network"`     // 通讯协议，默认为 tcp
	Password    string   `mapstructure:"password"`    // redis鉴权密码
	Db          int      `mapstructure:"db"`          // 数据库
	IdleTimeout int      `mapstructure:"idleTimeout"` // 空闲连接的超时时间，超过该时间则关闭连接。单位为秒。默认值是5分钟。值为0时表示不关闭空闲连接。此值应该总是大于redis服务的超时时间。
